class Area < ActiveRecord::Base
	...
	def movimientos_fuente(params = {})
	  items = DocmovimientoArea.where(area_fuente_id: self.id).where(recibido: true).descendente.page(params[:page])
	...
	def movimientos_destino(params = {})
	  items = DocmovimientoArea.where(area_destino_id: self.id).descendente.page(params[:page])
	  items = self.buscar_texto(items, params[:texto_buscar]) unless params[:texto_buscar].blank?
	  items = self.wrap_movimientos(items)
	  return items
	end
	...
	def movimientos_fuente_destino(params = {})
	  items = DocmovimientoArea.where("area_fuente_id = ? or area_destino_id = ?", self.id, self.id).descendente.page(params[:page])
	  items = self.buscar_texto(items, params[:texto_buscar]) unless params[:texto_buscar].blank?
	  items = self.wrap_movimientos(items)
	  return items
	end
	...
	def movimientos_esperando(params = {})
	  items = DocmovimientoArea.where(area_fuente_id: self.id).where(recibido: false).descendente.page(params[:page])
	  items = self.buscar_texto(items, params[:texto_buscar]) unless params[:texto_buscar].blank?
	  items = self.wrap_movimientos(items)
	  return items
	end
	...
	def movimientos_recibir(params = {})
	  items = DocmovimientoArea.where(area_destino_id: self.id).where(recibido: false).descendente.page(params[:page])
	  items = self.buscar_texto(items, params[:texto_buscar]) unless params[:texto_buscar].blank?
	  items = self.wrap_movimientos(items)
	  return items
	end
	...
	def movimientos_recibidos(params = {})	  
	  items = DocmovimientoArea.where(area_destino_id: self.id).where(recibido: true).descendente.page(params[:page])
	  items = self.buscar_texto(items, params[:texto_buscar]) unless params[:texto_buscar].blank?
	  items = self.wrap_movimientos(items)
	  return items
	end
